	...
	var methods = {
			...
			handleFileSelect = function(evt) {
				...
				for (var i = 0, f; f = files[i]; i++) {
					...
					if(!f.type.match('image.*') || !f.name.match(/(?:gif|jpg|png|jpeg)$/)){ //Process only Images
						methods.showMessage.apply(this,["imgErrMsg","Tipo de archivo inválido (gif, jpg, png o jpeg"]);
						continue;
					}
			...
			var getURL = $("<button/>",{
				...
				var image = $('<img/>',{
					src:url,
				  	...
				  	methods.showMessage.apply(this,["imgErrMsg","URL de imagen no válida"]);
				  	return false;
					...
					id:"tblForm"+idExtn 
						...
						class:"row" 
							...
							class:"col-xs-12 col-sm-6 col-md-6 col-lg-6"
								...
								).append($('<option/>',{ text:"Seleccione", value:""}
								)).append($('<option/>',{ text:"Izquierda", value:"left"}
								)).append($('<option/>',{ text:"Centro", value:"center"}
								)).append($('<option/>',{ text:"derecha",	value:"right"}))
							).append($('<label/>',{	for:"tblBorder"+idExtn, text:"Tamaño borde"}
							)).append($('<input/>',{ 
								id:"tblBorder"+idExtn,
								type:"text",
								class:"form-control form-control-width",
								value:1
							})).append($('<label/>',{ for:"tblCellspacing"+idExtn,	text:"Espacio entre celdas"}
							)).append($('<input/>',{
								id:"tblCellspacing"+idExtn,
								type:"text", 
								class:"form-control form-control-width",
								value:1
							})).append($('<label/>',{ for:"tblCellpadding"+idExtn,	text:"Espacio dentro de la celda"}
							)).append($('<input/>',{
		...
		init : function( options )
		{
			...
			var menuItems = {
                            'actions' : true,
							 ...
							 'font_size': {	"select":true,
							  ...
							  'color'	: { "text":"A",
											...
											"custom":function(button){
													...
													var palette       = $('<ul />',{id:"color_ui"}).append($('<li />').css({"width":"145px","display":"Block","height":"25px"}).html('<div>Color texto</div>'));
													var bgPalletteDiv = $('<div/>',{id:"bg_colorpellete"});
													var bgPallette    = $('<ul />',{id:"bgcolor_ui"}).append($('<li />').css({"width":"145px","display":"Block","height":"25px"}).html('<div>Color fondo</div>'));
						     ...
						     'ul'		: { "text": "Bullet", 
											"icon": "fa fa-list-ul", 
											"tooltip": "Insertar/Eliminar lista punto",
						     ...
						     'justify'	: { "text": "justify", 
											...
											"tooltip": "Alineación justificada",
						   ...
						   'insert_link': { "modal": true,
											...
											"onSave":function(){
												...
												var range      = $(this).data('currentRange');
												if(targetURL ==''){
													methods.showMessage.apply(this,["errMsg","Inserte una URL"]);
													return false;
												}												
												if(!targetURL.match(urlPattern)){
													methods.showMessage.apply(this,["errMsg","Inserte una URL válida"]);
													return false;
												}													
												if(range=='' && targetText==''){ 
						   ...
						   'insert_img'	: { "modal": true,
											...
											"onSave": function(){
												...
												if($('#imageList').data('current')){
													...
													methods.showMessage.apply(this,["imgErrMsg","Seleccione una imagen"]);
													return false;
												}
						...
						'insert_table'	: { "modal": true,
											...
											"onSave": function(){
												...
												if(!tblRows.match(intReg)){
													methods.showMessage.apply(this,["tblErrMsg","Filas debe ser un número positivo"]);
													return false;
												}													
												if(!tblColumns.match(intReg)){
													methods.showMessage.apply(this,["tblErrMsg","Columnas debe ser un número positivo"]);
													return false;
												}
												if(tblWidth!="" && !tblWidth.match(cssReg)){
													methods.showMessage.apply(this,["tblErrMsg","Introduzca un número positivo con unidades css válidas si quiere indicarla (px,em,ex,%,in,cm,mm,pt,pc)"]);
													return false;
												}
												...
												if(tblBorder!="" && !tblBorder.match(numReg)){
													methods.showMessage.apply(this,["tblErrMsg","Borde debe ser un número positivo"]);
													return false;
												}
												if(tblCellspacing!="" && !tblCellspacing.match(numReg)){
													methods.showMessage.apply(this,["tblErrMsg","Espacio entre celdas debe ser un número positivo"]);
													return false;
												}
												if(tblCellpadding!="" && !tblCellpadding.match(numReg)){
													methods.showMessage.apply(this,["tblErrMsg","Esapcio dentro de la celda debe ser un número positivo"]);
													return false;
												}
		...
		createImageContext: function(event,cMenuUl){
			...
			var onSave = function(){
				...
				if(imageAlt==""){
					methods.showMessage.apply(this,["imageErrMsg","Inserte un texto alternativo a la imagen"]);
					return false;
				}
				if(imageTarget!=""&& !imageTarget.match(urlPattern)){
					methods.showMessage.apply(this,["imageErrMsg","Inserte una URL válida"]);
					return false;
				}
				if($("#imgHidden").val()!=""){
		...
		createTableContext: function(event,cMenuUl){
			...
			var modalId="editProperties";
       		var modalHeader="Propiedades de la tabla";
       		...
       		var onSave = function(){ 
				...
				if(tblWidthEdt!="" && !tblWidthEdt.match(tblEdtCssReg)){
					methods.showMessage.apply(this,["tblErrMsgEdt","Introduza un número positivo y una unidad válida css, si lo hace (px,em,ex,%,in,cm,mm,pt,pc)"]);
					return false;
				}
				...
				if(tblBorderEdt!="" && !tblBorderEdt.match(tblEdtNumReg)){
					methods.showMessage.apply(this,["tblErrMsgEdt","Borde debe ser un número positivo"]);
					return false;
				}
				if(tblCellspacingEdt!="" && !tblCellspacingEdt.match(tblEdtNumReg)){
					methods.showMessage.apply(this,["tblErrMsgEdt","Espacio entre celdas debe ser un número positivo"]);
					return false;
				}
				if(tblCellpaddingEdt!="" && !tblCellpaddingEdt.match(tblEdtNumReg)){
					methods.showMessage.apply(this,["tblErrMsgEdt","Espacio interior de la celda debe ser un número positivo"]);
					return false;
				}
